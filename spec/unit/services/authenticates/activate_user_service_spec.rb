...
RSpec.describe Authenticates::ActivateUserService do
  ...
  context "when try to activate a user by using invitation code" do
    ...
    context "when using valid code" do
      before(:each) do
        out = Authenticates::ActivateUserService.new.execute(@user, @code)
        @status = out[:status]
        @user.reload
      end
    ...
    context "when using invalid code" do
      it "should not activate a user" do
        out = Authenticates::ActivateUserService.new.execute(@user, @code + "invalid")
        @user.reload
    ...
    context "when using expired code" do
      it "should not activate a user" do
        ...
        Authenticates::CreateInvitationService.new.execute(nil, @user)
        out = Authenticates::ActivateUserService.new.execute(@user, @code)
        @user.reload
