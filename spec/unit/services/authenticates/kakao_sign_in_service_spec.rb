...
RSpec.describe Authenticates::KakaoSignInService do
  ...
  it "should lead invitation page if not found a user matching with auth hash" do
    out = Authenticates::KakaoSignInService.new(@club).execute(@session, @auth_hash)
  ...
  it "should create user session" do
    user = FactoryGirl.create(:user, @user_params.merge(club: @club))
    out = Authenticates::KakaoSignInService.new(@club).execute(@session, @auth_hash)
  ...
  it "should update user extra_info" do
    user = FactoryGirl.create(:user, @user_params.merge(club: @club))
    Authenticates::KakaoSignInService.new(@club).execute(@session, @auth_hash)
    user.reload
